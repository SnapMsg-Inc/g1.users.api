...
async def create_user(*, db: Session = Depends(get_db), uid: str, user: UserCreate):
	crud.create_user(db, uid, user)
	return {"message": "user created"}


@app.get("/users", response_model=List[User])
async def get_users(*, db: Session = Depends(get_db), user: UserRead = Depends(), limit: int = Query(default=100, le=100), page: int = 0):
	print(f"LIMIT {limit}")
...
@app.patch("/users/{uid}")
async def update_user(*, db: Session = Depends(get_db), uid: str, user: UserUpdate):
...
async def get_recommended(*, db: Session = Depends(get_db), uid: str) -> List[User]:
	return {"message": "recommended users"}


@app.get("/users/{uid}/followers", response_model=List[User])
def get_followers(*, db: Session = Depends(get_db), uid: str, limit: int = Query(default=100, le=100), page: int = 0):
	return crud.read_followers(db, uid, limit, page)
	

@app.get("/users/{uid}/follows", response_model=List[User])
def get_follows(*, db: Session = Depends(get_db), uid: str, limit: int = Query(default=100, le=100), page: int = 0):
...
async def follow_user(*, db: Session = Depends(get_db), uid: str, otheruid: str):
