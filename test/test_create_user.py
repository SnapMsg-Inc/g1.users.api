...
def test_create_user_happy_path(db: Session, client: TestClient, cleanup_db):
    # Arrange
    uid = "unique_id"
    ...
    db_user = UserCreate.model_validate(db_user)

    assert res.status_code == 201
    assert db_user == test_user
    

def test_create_user_uid_already_exists(client: TestClient, cleanup_db):
    # Arrange
    uid = "unique_id"
    test_user = sample_create_user()
    res = client.post(f"/users/{uid}", json=test_user.model_dump())
...
def test_create_user_email_already_exists(client: TestClient, cleanup_db):
    # Arrange
    uid = "unique_id_0"
    test_user = sample_create_user()
    res = client.post(f"/users/{uid}", json=test_user.model_dump())
...
def test_create_user_nick_already_exists(client: TestClient, cleanup_db):
    # Arrange
    uid = "unique_id_0"
    test_user = sample_create_user()
